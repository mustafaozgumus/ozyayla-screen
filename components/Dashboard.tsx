...
const Dashboard: React.FC = () => {
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden flex flex-col font-sans text-slate-100 selection:bg-none cursor-none">
      ...
      <main className="relative z-10 flex-1 px-4 pb-4 pt-1 overflow-hidden" style={{ zoom: `${layout.dashboardZoom / 100}` }}>
        <div className="w-full h-full max-w-[98%] mx-auto grid grid-cols-10 gap-3">
          
          {/* Sol Kolon */}
          <div className="col-span-2 flex flex-col gap-3 h-full overflow-hidden">
            <div style={{ height: `${layout.weatherHeight}%` }} className="shrink-0"><WeatherCard /></div>
            <div style={{ height: `${layout.clockHeight}%` }} className="shrink-0"><ClockCard /></div>
            <div className="flex-1 min-h-0"><Birthdays /></div>
          </div>

          {/* Orta Kolon */}
          <div className="col-span-6 flex flex-col gap-3 h-full overflow-hidden">
            <div style={{ height: `${layout.newsHeight}%` }} className="w-full shrink-0"><NewsSlider /></div>
            <div className="flex-1 min-h-0"><Schedule /></div>
          </div>

          {/* Sağ Kolon */}
          <div className="col-span-2 flex flex-col gap-3 h-full overflow-hidden">
             <div style={{ height: `${layout.dutyHeight}%` }} className="shrink-0"><DutyTeachers /></div>
             <div style={{ height: `${layout.announceHeight}%` }} className="shrink-0"><AnnouncementsList /></div>
